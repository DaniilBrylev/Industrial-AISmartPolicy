...
class AIService:
    ...
    async def generate_policy_section(self, report: dict[str, Any]) -> str:
        ...
        ib_extra = ""
        if section == "ib_system_description":
            ib_extra = (
                "\n\nРаздел «Описание системы обеспечения ИБ»: опиши организационно-техническую систему ИБ "
                "предприятия (подсистемы управления, защиты, мониторинга, реагирования/восстановления и цикл "
                "непрерывного улучшения). Сохрани все факты и цифры из черновика.\n"
                "Запрещено упоминать программные продукты разработки, ИИ, автоматическую генерацию документа, "
                "анкеты, rule-based анализ и любые внутренние названия систем — текст должен читаться как фрагмент "
                "нормативного документа организации, а не как описание ПО."
            )
